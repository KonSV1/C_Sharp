...
else
{
    ...
    else
    {
        ...
        else
        {
            if ((X2==X3) || (X1==X4))
            {
                if (X2==X3)Console.Write($"Общаа чточка {X2}");
                else
                {
                 Console.Write($"Общаа чточка {X1}");
                }
            }
            else
            {
            if((X1<X3) & (X2>X3))Console.Write($"Граница пересечения отрезков ({X3}; {X2})");
            if((X3<X1) & (X2>X4))Console.Write($"Граница пересечения отрезков ({X1}; {X4})");
            if((X4<X2) & (X3>X1))Console.Write($"Граница пересечения отрезков ({X3}; {X4}). Отрезок 1 полностью включает в себя отрезок 2");
